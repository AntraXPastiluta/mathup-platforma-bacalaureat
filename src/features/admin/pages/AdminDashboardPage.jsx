        ...
        >
          {ADMIN_SECTIONS.map((section) => (
            <button
              ...
              className={`group relative overflow-hidden rounded-3xl p-5 text-left transition-all duration-300 ${adminSection === section.id
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className={`p-2.5 rounded-2xl transition-all duration-300 ${adminSection === section.id
                    ? 'bg-white/20 shadow-inner'
                    : 'bg-slate-200/60 dark:bg-white/5 group-hover:bg-primary/10 group-hover:scale-110'
                  }`}>
                  <section.icon className={`size-6 ${adminSection === section.id ? 'text-white' : 'text-slate-700 dark:text-slate-400 group-hover:text-primary'
                    }`} />
                </div>
              ...
              <div className="space-y-1">
                <h3 className={`text-sm font-black uppercase tracking-widest leading-none ${adminSection === section.id ? 'text-white' : 'text-slate-800 dark:text-slate-200'
                  }`}>
                  {section.label}
                </h3>
                <p className={`text-[10px] font-bold leading-tight ${adminSection === section.id ? 'text-white/90' : 'text-slate-500'
                  }`}>
                  {section.description}
                </p>
              </div>
        ...
        {adminSection === 'roadmaps' ? (
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
            <div className="lg:col-span-4 space-y-4">
              ...
              <motion.div className="space-y-2 rounded-3xl border border-slate-300 bg-white p-3 dark:border-white/10 dark:bg-white/5">
                {roadmaps.length === 0 ? (
                  ...
                  roadmaps.map((roadmap) => (
                    <button
                      ...
                      className={`w-full rounded-2xl border px-4 py-3 text-left transition-all ${selectedRoadmapId === roadmap.id ? 'border-primary bg-primary/15 shadow-md ring-1 ring-primary/20' : 'border-transparent hover:bg-slate-100 dark:hover:bg-white/5'}`}
                    >
                ...
                <div className="rounded-3xl border border-slate-300 bg-white p-8 shadow-xl dark:border-white/10 dark:bg-white/5 space-y-6">
                  ...
                  <div className="space-y-2">
                    {(selectedRoadmap?.study_roadmap_steps ?? []).map((step, index) => (
                      <div key={step.id} className="flex items-center justify-between rounded-2xl border border-slate-300 px-4 py-3 dark:border-white/10">
                        <div>
              ...
              <AnimatePresence mode="wait">
                {!selectedLesson && !isCreating ? (
                  ...
                  >
                    ...
                    <Button variant="outline" onClick={() => setIsCreating(true)} className="mt-2 rounded-full border-slate-300 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5 shadow-sm text-slate-600 dark:text-slate-300">
                      ...
                      {!isEditingMetadata && (
                        <div className="flex items-center gap-1 mt-8 p-1.5 bg-slate-200 dark:bg-black/20 rounded-2xl border border-slate-300 dark:border-white/5 w-fit shadow-inner">
                          {[
                            ...
                            >
                              <tab.icon className={`size-3.5 ${activeTab === tab.id ? 'text-primary' : 'text-slate-500 dark:text-slate-600'}`} />
                              {tab.label}
                            </button>
                          ))}
                        </div>
