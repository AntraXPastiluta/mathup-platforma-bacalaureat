...
export const USER_MESSAGES = {
  ...
  authOAuth: 'Autentificarea cu Google nu a reușit. Încearcă din nou.',
...
const SAFE_MESSAGE_PREFIXES = [
  ...
  'Ai atins limita de 3 exporturi',
  'Nu am putut genera exportul',
  'Exportul datelor',
  'Acces neautorizat',
  'Titlul',
  'Toate câmpurile',
...
export function toAuthOAuthError(error) {
  const message =
    error instanceof Error
      ? error.message
      : typeof error === 'string'
  ...
  if (/popup|blocked|closed/i.test(message)) {
    return 'Fereastra de autentificare Google a fost închisă. Încearcă din nou.'
  }
  if (/provider.*disabled|oauth.*disabled|unsupported provider/i.test(message)) {
    return 'Autentificarea cu Google nu este activă momentan. Contactează suportul.'
  }
  if (/already registered|user already exists|email.*taken/i.test(message)) {
    return 'Există deja un cont cu acest email. Încearcă autentificarea cu parolă.'
  }

  return toUserFacingError(error, USER_MESSAGES.authOAuth)
}
